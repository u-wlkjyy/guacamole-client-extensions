...
package org.apache.guacamole.auth.filetoken.connection;
...
public class ConnectionService {
    ...
    public GuacamoleTunnel connect(UserData.Connection connection,
        ...
        // Get default guacd connection parameters
        String hostname = proxyConfig.getHostname();
        int port = proxyConfig.getPort();
        GuacamoleProxyConfiguration.EncryptionMethod encryptionMethod = proxyConfig.getEncryptionMethod();

        // Override with dynamic proxy settings if present
        if (connection.getGuacdHostname() != null) {
            hostname = connection.getGuacdHostname();
            if (connection.getGuacdPort() != null) {
                port = connection.getGuacdPort();
            }
            if (connection.getGuacdEncryption() != null) {
                try {
                    encryptionMethod = GuacamoleProxyConfiguration.EncryptionMethod.valueOf(
                            connection.getGuacdEncryption().toUpperCase());
                } catch (IllegalArgumentException e) {
                    logger.warn("Invalid encryption method \"{}\" for connection.", connection.getGuacdEncryption());
                }
            }
        }
