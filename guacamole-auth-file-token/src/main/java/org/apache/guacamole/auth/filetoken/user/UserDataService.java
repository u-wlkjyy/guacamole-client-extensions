...
import org.apache.guacamole.auth.filetoken.ConfigurationService;
...
public class UserDataService {
    ...
    public UserData fromCredentials(Credentials credentials) {
        ...
        try {
            File authDir = confService.getAuthDir();
            File tokenFile = new File(authDir, token + ".json");
            
            // Security check: ensure the file is actually inside the authDir
            if (!tokenFile.getCanonicalPath().startsWith(authDir.getCanonicalPath())) {
                logger.warn("Attempted directory traversal with token: {}", token);
                return null;
            }
            ...
            UserData userData = mapper.readValue(tokenFile, UserData.class);
            if (userData.isExpired())
                return null;
            return userData;

        } catch (Exception e) {
            logger.error("Error validating file token: {}", e.getMessage());
